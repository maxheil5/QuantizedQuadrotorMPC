...
from quantized_quadrotor_sitl.experiments.runtime_reference import (
    ...
    build_sitl_identification_reference_v7,
...
def test_sitl_identification_reference_v7_is_deterministic_and_biases_forward_excitation():
    state0 = initial_state()
    reference_a = build_sitl_identification_reference_v7(
        state0,
        reference_duration_s=24.0,
        sim_timestep=1.0e-3,
        rng=np.random.default_rng(2141444),
    )
    reference_b = build_sitl_identification_reference_v7(
        state0,
        reference_duration_s=24.0,
        sim_timestep=1.0e-3,
        rng=np.random.default_rng(2141447),
    ...
    assert np.min(reference_a[2, :]) >= state0[2] - 1.0e-9
    assert np.max(reference_a[2, :]) <= state0[2] + 0.85 + 1.0e-9
    assert np.min(reference_a[0, :]) >= state0[0] - 0.80 - 1.0e-9
    assert np.max(reference_a[0, :]) <= state0[0] + 0.80 + 1.0e-9
    assert np.min(reference_a[1, :]) >= state0[1] - 0.45 - 1.0e-9
    assert np.max(reference_a[1, :]) <= state0[1] + 0.45 + 1.0e-9
    assert np.max(np.abs(reference_a[3:6, :])) > 0.35
    assert np.max(np.abs(reference_a[15:18, :])) <= 1.0e-9
...
def test_runtime_reference_selector_supports_sitl_identification_v7_mode():
    state0 = initial_state()
    rng_selected = np.random.default_rng(2141444)
    rng_expected = np.random.default_rng(2141444)
    selected = build_runtime_reference(
        state0,
        reference_mode="sitl_identification_v7",
        reference_duration_s=24.0,
        sim_timestep=1.0e-3,
        rng=rng_selected,
    )
    expected = build_sitl_identification_reference_v7(state0, 24.0, 1.0e-3, rng_expected)
    npt.assert_allclose(selected, expected)
