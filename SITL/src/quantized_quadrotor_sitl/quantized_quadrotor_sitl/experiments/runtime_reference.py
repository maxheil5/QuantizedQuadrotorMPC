...
def build_sitl_identification_reference_v4(
    initial_state: np.ndarray,
    reference_duration_s: float,
    sim_timestep: float,
    rng: np.random.Generator,
    ...
    mask, tau, envelope = _windowed_segment_profile(sample_times, 7.0, 13.0)
    if np.any(mask):
        x_offset_local = envelope * (
            0.38 * np.sin(4.0 * np.pi * tau + phase_x) + 0.12 * np.sin(12.0 * np.pi * tau + 0.25)
        )
        y_offset_local = envelope * 0.06 * np.sin(8.0 * np.pi * tau + phase_y)
        x_offset_world, y_offset_world = _rotate_planar_offsets(x_offset_local, y_offset_local, initial_heading)
        position[0, mask] = x0 + x_offset_world
        position[1, mask] = y0 + y_offset_world
        position[2, mask] = hover_position[2] + envelope * (
            0.06 * np.sin(4.0 * np.pi * tau + phase_z) + 0.02 * np.sin(12.0 * np.pi * tau)
        )

    forward_times = np.array([13.0, 14.0, 15.0, 16.0, 17.0, 18.0, 18.5], dtype=float)
    forward_points_local = [
        np.array([0.0, 0.0, 0.75], dtype=float),
        np.array([0.42, 0.05, 0.80], dtype=float),
        np.array([-0.42, -0.05, 0.70], dtype=float),
        np.array([0.30, 0.04, 0.78], dtype=float),
        np.array([-0.30, -0.04, 0.72], dtype=float),
        np.array([0.16, 0.02, 0.76], dtype=float),
        np.array([0.0, 0.0, 0.75], dtype=float),
    ]
    ...
    mask, tau, envelope = _windowed_segment_profile(sample_times, 18.5, 24.0)
    if np.any(mask):
        x_offset_local = envelope * (
            0.34 * np.sin(4.0 * np.pi * tau + phase_x) + 0.10 * np.sin(10.0 * np.pi * tau + 0.3)
        )
        y_offset_local = envelope * 0.10 * np.sin(8.0 * np.pi * tau + phase_y)
        x_offset_world, y_offset_world = _rotate_planar_offsets(x_offset_local, y_offset_local, initial_heading)
        position[0, mask] = x0 + x_offset_world
        position[1, mask] = y0 + y_offset_world
        position[2, mask] = hover_position[2] + envelope * (
            0.05 * np.sin(4.0 * np.pi * tau + phase_z) + 0.02 * np.sin(12.0 * np.pi * tau + 0.2)
        )
...
def build_sitl_identification_reference_v5(
    initial_state: np.ndarray,
    reference_duration_s: float,
    sim_timestep: float,
    rng: np.random.Generator,
    ...
    phase_x = 5.438391020962071
    phase_y = 0.3717766430357354
    phase_z = 0.9134815690436796

    mask, tau, envelope = _windowed_segment_profile(sample_times, 7.0, 11.0)
    if np.any(mask):
        x_offset_local = envelope * (
            0.44 * np.sin(4.0 * np.pi * tau + phase_x) + 0.14 * np.sin(10.0 * np.pi * tau + 0.4)
        )
        y_offset_local = envelope * 0.025 * np.sin(8.0 * np.pi * tau + phase_y)
        x_offset_world, y_offset_world = _rotate_planar_offsets(x_offset_local, y_offset_local, initial_heading)
        position[0, mask] = x0 + x_offset_world
        position[1, mask] = y0 + y_offset_world
        position[2, mask] = hover_position[2] + envelope * (
            0.06 * np.sin(4.0 * np.pi * tau + phase_z) + 0.02 * np.sin(10.0 * np.pi * tau)
        )

    forward_times = np.array([11.0, 12.0, 13.0, 14.2, 15.4, 16.4, 17.2], dtype=float)
    forward_points_local = [
        np.array([0.0, 0.0, 0.75], dtype=float),
        np.array([0.58, 0.02, 0.80], dtype=float),
        np.array([-0.58, -0.02, 0.70], dtype=float),
        np.array([0.46, 0.02, 0.79], dtype=float),
        np.array([-0.46, -0.02, 0.71], dtype=float),
        np.array([0.26, 0.01, 0.77], dtype=float),
        np.array([0.0, 0.0, 0.75], dtype=float),
    ]
    for idx in range(forward_times.size - 1):
        start_local = forward_points_local[idx]
    ...
    mask, tau, envelope = _windowed_segment_profile(sample_times, 17.2, 20.5)
    if np.any(mask):
        x_offset_local = envelope * (
            0.48 * np.sin(4.0 * np.pi * tau + phase_x) + 0.16 * np.sin(12.0 * np.pi * tau + 0.35)
        )
        y_offset_local = envelope * 0.03 * np.sin(8.0 * np.pi * tau + phase_y)
        x_offset_world, y_offset_world = _rotate_planar_offsets(x_offset_local, y_offset_local, initial_heading)
        position[0, mask] = x0 + x_offset_world
        position[1, mask] = y0 + y_offset_world
        position[2, mask] = hover_position[2] + envelope * (
            0.05 * np.sin(4.0 * np.pi * tau + phase_z) + 0.02 * np.sin(12.0 * np.pi * tau + 0.3)
        )

    mask, tau, envelope = _windowed_segment_profile(sample_times, 20.5, 24.0)
    if np.any(mask):
        x_offset_local = envelope * (
            0.36 * np.sin(4.0 * np.pi * tau + phase_x) + 0.10 * np.sin(12.0 * np.pi * tau + 0.25)
        )
        y_offset_local = envelope * 0.04 * np.sin(8.0 * np.pi * tau + phase_y)
        x_offset_world, y_offset_world = _rotate_planar_offsets(x_offset_local, y_offset_local, initial_heading)
        position[0, mask] = x0 + x_offset_world
        position[1, mask] = y0 + y_offset_world
        position[2, mask] = hover_position[2] + envelope * (
            0.04 * np.sin(4.0 * np.pi * tau + phase_z) + 0.015 * np.sin(12.0 * np.pi * tau + 0.1)
        )
...
def build_runtime_reference(
    ...
    if reference_mode == "sitl_identification_v4":
