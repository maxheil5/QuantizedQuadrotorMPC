...
for seed in "${SEEDS[@]}"; do
  ...
  cleanup_sitl_processes
  sleep 2
  if timeout --signal=INT --kill-after=10 "${RUN_TIMEOUT_SECONDS}" bash "${ROOT_DIR}/scripts/run_sitl_experiment.sh" "${CONFIG_PATH}"; then
    echo "=== Seed ${seed} completed ==="
  else
    status=$?
    if [[ "${status}" -eq 124 ]]; then
      echo "=== Seed ${seed} hit timeout after ${RUN_TIMEOUT_SECONDS}s; continuing after cleanup ===" >&2
    else
