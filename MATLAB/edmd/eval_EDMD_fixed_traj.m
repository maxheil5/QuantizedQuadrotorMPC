function [RMSE_val, X] = eval_EDMD_fixed_traj(X0,X,U,dt,EDMD,n_basis)
%% get EDMD matrices
A = EDMD.A;
B = EDMD.B;
C = EDMD.C;
...
theta_err = []; wb_err = [];
    ...
    % figure(1); hold on;
    % subplot(2,4,2)
    % plot3(x_true(1,:), x_true(2,:), x_true(3,:),'color','b'); hold on
    % plot3(x_pred(1,:), x_pred(2,:), x_pred(3,:), 'color','r','linestyle','--'); hold on
